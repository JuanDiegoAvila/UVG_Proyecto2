...
def crear():
    ...
    if existe.single()[0] == 0:
        ...
        for i in generos:
            if (temp.count(i) < 1):
                comando_generos = "CREATE (a:Genero {nombre: '" + i[0] + "'}) RETURN a"
        ...
        for i in artistas:
            if (temp.count(i) < 1):
                comando_artista = "CREATE (a:Artista {nombre: '%s'}) RETURN a" % i[0]
        ...
        for i in estados_de_animo:
            if (temp.count(i) < 1):
                comando_animo = "CREATE (a:Animo {nombre: '%s'}) RETURN a" % i[0]
                comando(sesion, comando_animo)

            temp.append(i)

        for i in canciones:
            comando_canciones = "CREATE (c:Cancion {nombre: '%s'}) RETURN c" % i[0]
        ...
        for t in inf_temporal: #lista que contiene lista con toda la informacion de cada cancion.
            ...
            comando_canta = "MATCH (a:Artista {nombre:'%s'}),(b:Cancion{nombre:'%s'}) MERGE (a)-[r:Canta]->(b)" % (t[0],t[3])
            comando(sesion,comando_canta)

            #se define el genero de la cancion.
            comando_g = "MATCH (a:Cancion {nombre:'%s'}),(b:Genero{nombre:'%s'}) MERGE (a)-[r:Pertenece_a]->(b)" % (t[3], t[1])
            comando(sesion, comando_g)

            comando_an = "MATCH (a:Cancion {nombre:'%s'}),(b:Animo{nombre:'%s'}) MERGE (a)-[r:Genera]->(b)" % (t[3], t[2])
            comando(sesion, comando_an)

            comando_crea = "MATCH (a:Artista {nombre:'%s'}),(b:Genero{nombre:'%s'}) MERGE (a)-[r:Crea]->(b)" % (t[0], t[1])
